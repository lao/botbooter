...
func loggingMiddleware(ctx context.Context, bot *botbooter.Bot, message *botbooter.Message, next botbooter.CommandHandler) {
	...
	// must fetch via botbooter.SlackClient(bot).GetFileContext (it needs the bot
	// token); Telegram resolves a link that EMBEDS the bot token in plaintext —
	// secret, so this demo does not print it; WhatsApp returns a link you fetch
	// with the Cloud API bearer token; CLI returns a local file path.
